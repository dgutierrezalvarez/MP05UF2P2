...
public class HashTable {
    ...
    //Los items del put no incrementan
    public void put(String key, String value) {
        int hash = getHash(key);
        final HashEntry hashEntry = new HashEntry(key, value);
        ...
        if(entries[hash] == null) {
            ...
            // Crea un array para evitar que haya colisiones
            HashEntry temp = entries[hash];

            // Para meter el valor en diferentes huecos
            while(temp.next != null) {
                temp = temp.next;
            }
            // Para sobreesbrir claves
    ...
    //añadimos un try para cuando el valor sea superior hacer que funcione
    public String get(String key) {
        int hash = getHash(key);
        try {
            if (entries[hash] != null) {
    ...
    private HashEntry getHashEntry(String key, HashEntry temp) {
        while (!temp.key.equals(key)) //seleccionar linea, refactor, extract
            temp = temp.next;
        return temp;
    }
